...
// Memory for extrinsic information (EI) and LDPC output
	...
	{
		printf("khanh's 10/12 modulation code\n");
		//fprintf(fber, "Seungmin's 6/9 modulation code\n");
	}
	else if (Encoder == 6)
	{
...
void PRML_Convolution(double** trellis, double* Target_Coef, int Target_Order)
{
	...
	for (i = 0; i < Target_Order; i++)
		temp_trel_a[i] = 0;
	// Initialize to 0
	//, std::cout << temp_trel_a[i] << std::endl
	//std::cout << "In total_states" << std::endl;
	for (i = 0; i < total_states; i++)
	{
		//std::cout << "In total_states, for temp_trel_a" << std::endl;
		for (k = 0; k < Target_Order; k++) // Target_Order = 2, mảng temp_trel_a này có 2 value nha
			temp_trel_a[k] = (i / (int)pow((double)M_ary, k)) % M_ary;
		// , std::cout << temp_trel_a[k] << std::endl;
		...
		//std::cout << "Trellis after each loop, in each state" << std::endl;
		//for (int i = 0; i < 16; i++) {
		//	printf("Row %2d: ", i + 1);
		//	for (int j = 0; j < 1029; j++) { // Chỉ log tối đa 10 cột
		//		printf("%3d ", (int)trellis[i][j]);
		//	}
		//	printf("\n");
		//}
